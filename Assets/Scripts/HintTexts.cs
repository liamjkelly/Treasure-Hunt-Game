...
public class HintTexts : MonoBehaviour {
	...
	public GameObject player, fish, researchNotes, researcher, notes;
	...
	void Update () {
		...
		if (Vector3.Distance(player.transform.position, fish.transform.position) < 10f) { //checks if first clue found
			hintText.text = "This fish is pointed to the East. Better see what's over there.";
		}

		if (Vector3.Distance (player.transform.position, researchNotes.transform.position) < 10f) { //checks if second clue found
			hintText.text = "These notes talk about a research station to the South. Maybe they know something.";
		}

		if (Vector3.Distance (player.transform.position, researcher.transform.position) < 10f) { //checks if third clue found
